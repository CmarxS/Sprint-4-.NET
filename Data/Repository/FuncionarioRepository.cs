...
namespace MottoMap.Data.Repository
{
    ...
    public class FuncionarioRepository : BaseRepository<FuncionarioEntity>, IFuncionarioRepository
    {
        ...
        public async Task<bool> EmailExistsAsync(string email, int? idFuncionarioAtual = null)
        {
            ...
            {
                // Para atualizações - verificar se existe outro funcionário com o mesmo email
                var count = await _dbSet
                    .Where(f => f.Email == email && f.IdFuncionario != idFuncionarioAtual.Value)
                    .CountAsync();
                return count > 0;
            }
            else
            {
                // Para criações - verificar se existe qualquer funcionário com o email
                var count = await _dbSet
