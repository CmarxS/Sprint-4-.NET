...
// Registro do serviço de ML.NET
builder.Services.AddSingleton<MotoMaintenancePredictionService>();
...
- **Machine Learning**: Previsão de manutenção com ML.NET
...
## 🔐 Autenticação
Para acessar os endpoints (exceto /health e /swagger), adicione o header:
```
...
- ✅ **ML - Previsão de Manutenção**: Análise inteligente com Machine Learning
...
- **ML**: ML.NET para previsão de manutenção
- **Documentação**: Swagger/OpenAPI
- **Segurança**: API Key Authentication
",
    });

    // Configurar segurança no Swagger
    c.AddSecurityDefinition("ApiKey", new OpenApiSecurityScheme
    {
    Description = "API Key necessária para acessar os endpoints. Adicione: X-API-Key: MottoMap2024SecureKey",
        Name = "X-API-Key",
...
// HABILITADO PARA TODOS OS AMBIENTES (APENAS PARA TESTES)
app.UseSwagger();
app.UseSwaggerUI(c =>
{
...
// Middleware de API Key (deve vir antes de UseAuthorization)
